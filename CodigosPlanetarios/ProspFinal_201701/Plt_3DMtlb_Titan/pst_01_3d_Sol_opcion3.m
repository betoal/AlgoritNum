...
% prog pst_03d_Sol.m
%
%  Simulacisn 3D del movimiento de los planetas del Sistema Planetario
%  Solar
%  Por R.G.G.H.
...
cuando = 4000; %2260
GuardarCadaCuantos = 5; %% cada cuantos calculos (daos por el valor de t)  hay que guardar las posiciones
N =4.5*86400/deltaT; %cantidad de calculos a llevarse a cabo
%N = cuando*GuardarCadaCuantos;
N_GuardaPosiciones = 1 + N / GuardarCadaCuantos;
% ===================== Variables para distancias mmnimas del Apolo a la
...
if Alternativa == 1
  ...
  T8_Apollo = 2265 * GuardarCadaCuantos*deltaT; %%2257
  T9_Apollo = T8_Apollo + 10 * deltaT;
  Acel8 = 9.8 * 0.9 ;%0.758;

  %% !!! FRENA !!!
  T10_Apollo = 4000 * GuardarCadaCuantos*deltaT; %%4800
  T11_Apollo = T10_Apollo + 20 * deltaT;
  Acel10 = - 9.8 * 1.0;   
...
incl = 5.145*pi/180.0; %% inclinacisn del plano de traslacisn de la Luna en grados
c5 = cos(incl);
s5 = sin(incl);
...
% ==========================================================
% se almacenan las posiciones iniciales del sol, planetas y demas cuerpos
% ==========================================================
...
for t = 1:N
  ...
  % ================ Ejecucisn de las alternativas de navegacisn ================
  if Alternativa == 1
    ...
    % Actualizacisn de las distancias mmnimas del Apolo a Tierra al inicio,
  ...
  %
  % ================ Fin del Ciclo de Calculo de la simulacisn ============
  %
  end
  
 
% ================ Fin de la ejecucisn de las alternativas de navegacisn ========= 
  for i = 1:np
...
% ================================================================
% Se grafican las trayectorias de las srbitas de los cuerpos celestes
% ================================================================

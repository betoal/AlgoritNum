...
% =======================================================================
G = 6.67384E-11;
deltaT = 3*60;  % deltaT en segundos
T = 0.0; % Tiempo transcurrido
N = 0.5*86400/deltaT; %cantidad de cálculos a llevarse a cabo
...
% indices de los móviles para poder preguntar por ellos al graficarlos
...
if ind_Titan > 0
%
% Cassini alrededor de Titán
%
   alfa_cassini_1 = 0.0/180.0*pi; beta_cassini_1 = 0.0/180.0*pi;
   c_alfa_cassini_1 = cos( alfa_cassini_1 ); s_alfa_cassini_1 = sin( alfa_cassini_1);
   c_beta_cassini_1 = cos( beta_cassini_1 ); s_beta_cassini_1 = sin( beta_cassini_1);
   num_ind = num_ind+1; ind_Cassini_Titan   = num_ind; plt = [plt; ClsMovil_2D8('Cassini_Titan',4e3,[1514521447818.79,-251697512.900079,0]+ ...
                                                                                              6.4375E6*[1,0,0], ...
                                                                                              [1147.33095066515,14816.1960418493,0]+ ...
                                                1.1808E3*[-s_alfa_cassini_1,c_alfa_cassini_1*s_beta_cassini_1,c_alfa_cassini_1*c_beta_cassini_1])];
end

if ind_Saturno > 0
 %
 % Cassini alrededor de Saturno "Grand Final"
 %
 alfa_cassini_2 = 45.0/180.0*pi; beta_cassini_2 = 30.0/180.0*pi;
 c_alfa_cassini_2 = cos( alfa_cassini_2 ); s_alfa_cassini_2 = sin( alfa_cassini_2);
 c_beta_cassini_2 = cos( beta_cassini_2 ); s_beta_cassini_2 = sin( beta_cassini_2);
 num_ind = num_ind+1; ind_Cassini_GrandFinal   = num_ind; plt = [plt; ClsMovil_2D8('Cassini_GrandFinal',4e3, ...
                                                                 [1513325782874.55,0,0]  ...
                                                      + 63583000.00 * [1,0,0],          ...
                                                       [0, 9365.9101754812,0]             ...
                                                      + 24434.15 * [-c_alfa_cassini_2*c_beta_cassini_2,c_alfa_cassini_2*s_beta_cassini_2,s_alfa_cassini_2])];
end
...
if ind_Dione > 0
  ...
  % Helena
  %
  num_ind = num_ind+1; ind_Helena = num_ind;plt = [plt;  ClsMovil_2D8( 'Helena', 100e3, ...
                                                1.0e+12 *[1.512995757097856,-0.000182582863161 ,0], ...
                                                1.0e+03 *[4.852997258557223,0.593924887152254,0])]; 
                                            
  %
  % Polux
  %
  num_ind = num_ind+1; ind_Polux = num_ind;plt = [plt;  ClsMovil_2D8( 'Polux', 100e3, ...
                                                1.0e+12 *[1.513332674365104,0.000377102138101 ,0], ...
                                                1.0e+04 *[ -1.002326073059485,0.954908390923888,0])]; 
...
if ind_Tetis > 0
  ...
  % Telesto
  %
  num_ind = num_ind+1; ind_Telesto = num_ind;plt = [plt;  ClsMovil_2D8( 'Telesto', 100e3, ...
                                                1.0e+12 *[1.513168404472592,0.000249063031692,0], ...
                                                1.0e+03 *[ -9.588825860591536,3.306905431406960,0])]; 
                                            
  %
  % Calipso
  %
  num_ind = num_ind+1; ind_Calipso = num_ind;plt = [plt;  ClsMovil_2D8( 'Calipso', 100e3, ...
                                                1.0e+12 *[1.513620166988118,0.000011762178257 ,0], ...
                                                1.0e+04 *[-0.045283909972296,2.069957933525577,0])]; 
...
% ==============================================================================
np = num_ind;
% =============== Para definir las órbitas "exactas" ======================
...
% ==========================================================
  if ind_Jano > 0 && ind_Epimeteo > 0
    dist_Min_Jano_Epimeteo = 1e20;
    T_dist_Min_Jano_Epimeteo = -1;
  end
for t = 1:N
  % ================================================================================================
  %                   Matriz de fuerzas
  ...
  %
  % Reviso colisiones entre Jano y Epimeteo
  %
     if ind_Jano > 0 & ind_Epimeteo > 0
       deltaPos = plt(ind_Jano).posicion - plt(ind_Epimeteo).posicion;
       dist_Jano_Epimeteo = sqrt( deltaPos * deltaPos');
       if dist_Jano_Epimeteo < dist_Min_Jano_Epimeteo
          dist_Min_Jano_Epimeteo = dist_Jano_Epimeteo;
          T_dist_Min_Jano_Epimeteo = t;
       end    
       if dist_Jano_Epimeteo < 400000
          disp(sprintf('Colision, dist %f al paso %d',dist_Jano_Epimeteo,t)) 
       end    
     end  
  %
...
if ind_Jano > 0 && ind_Epimeteo > 0
  disp(sprintf('La distancia minima entre Jano y Epimeteo fue de %f al tiempo %d',dist_Min_Jano_Epimeteo,T_dist_Min_Jano_Epimeteo))      
end   
...
for k = np:-1:1
  ...
  x = x';
  if k == ind_Sol
    plot3(x(:,1),x(:,2),x(:,3),'oy') 
  elseif k == ind_Saturno
